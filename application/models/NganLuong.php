...
class NganLuong 
{
	...
	public static $merchant_site_code = '36080'; //100001 chỉ là ví dụ, bạn hãy thay bằng mã của bạn

	// Mật khẩu giao tiếp giữa website của bạn và NgânLượng.vn.
	public static $secure_pass= '01265230779'; //d685739bf1 chỉ là ví dụ, bạn hãy thay bằng mật khẩu của bạn
	 ...
	 * @return string
	 */
	public static function buildCheckoutUrlExpand($return_url, $receiver, $transaction_info, $order_code, $price, $currency = 'vnd', $quantity = 1, $tax = 0, $discount = 0, $fee_cal = 0, $fee_shipping = 0, $order_description = '', $buyer_info = '', $affiliate_code = '')
	{	
		if($affiliate_code == "") 
			$affiliate_code = self::$affiliate_code;
		
		$arr_param = array(
			'merchant_site_code'=>	strval(self::$merchant_site_code),
		...
		$secure_code ='';
		$secure_code = implode(' ', $arr_param) . ' ' . self::$secure_pass;
	...
	public static function verifyPaymentUrl($transaction_info, $order_code, $price, $payment_id, $payment_type, $error_text, $secure_code)
	{
		...
		$str .= ' ' . strval(self::$merchant_site_code);
		$str .= ' ' . strval(self::$secure_pass);
