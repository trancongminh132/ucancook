...
class BaoKim
{
	...
	private static $baokim_url = 'https://www.baokim.vn/payment/customize_payment/order';

	// Mã merchante site 
	private static $merchant_id = '15834';	// Biến này được baokim.vn cung cấp khi bạn đăng ký merchant site
	
	// Mật khẩu bảo mật
	private static $secure_pass = '0b10f374daca014b'; // Biến này được baokim.vn cung cấp khi bạn đăng ký merchant site
	...
	public function createRequestUrl($order_id, $business, $total_amount, $shipping_fee, $tax_fee, $order_description, $url_success, $url_cancel, $url_detail)
	{
		...
		$params = array(
			'merchant_id'		=>	strval(self::$merchant_id),
		...
		$str_combined = self::$secure_pass.implode('', $params);
	...
	public function verifyResponseUrl($_GET = array())
	{
		...
		ksort($_GET);
		$str_combined = self::$secure_pass.implode('', $_GET);
